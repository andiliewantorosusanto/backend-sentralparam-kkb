...
public class JenisPerluasanRestController {
    ...
    public Map<String, Object> getAllDataJenisPerluasan(Principal principal, Pageable pageable,
        ...
        if (role.contains("USER")) {
            try {
                logger.info("Berhasil GET ALL DATA Jenis Perluasan");
                crunchifyMap.put("jenisPerluasan", jenisPerluasanRepository.findAll());
                crunchifyMap.put("code", "1");
            } catch (Exception e) {
                logger.error("ERROR");
                response.setStatus(400);
                crunchifyMap.put("code", "0");
                crunchifyMap.put("message", "Gagal membuka Jenis Perluasan!");
            }
            return crunchifyMap;
        } else {
