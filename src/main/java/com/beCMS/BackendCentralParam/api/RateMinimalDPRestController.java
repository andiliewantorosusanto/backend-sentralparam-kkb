...
@CrossOrigin(origins = "*")
public class RateMinimalDPRestController {

    Logger logger = LoggerFactory.getLogger(RateMinimalDPRestController.class);
    ...
    public Map<String, Object> getAllDataRateMinimalDP(Principal principal, Pageable pageable,
        ...
        if (role.contains("USER")) {
            try {
                logger.info("Berhasil GET ALL DATA RATE Minimal DP");
                crunchifyMap.put("dataRateMinimalDP", RateMinimalDPRepository.getListDataMinimalDP());
     ...
     public Map<String, Object> getAllDataApprovalrateKomponenPH(Principal principal, Pageable pageable,
         ...
         if (role.contains("USER")) {
             try {
                 logger.info("Berhasil GET ALL DATA RATE MINIMAL DP");
                 crunchifyMap.put("datarateKomponenPH", RateMinimalDPRepository.getListBucketApprovalMinimalDP());
                 crunchifyMap.put("code", "1");
             } catch (Exception e) {
                 logger.error("ERROR");
                 response.setStatus(400);
    ...
    public HashMap<String, String> insertBiayaProvisi(@RequestBody Map<String, Object> data, Principal principal,
        ...
        System.out.println("Masuk Input Minimal DP");
        modelMinimalDP tmp = new modelMinimalDP();
        ...
        tmp.setTujuan_penggunaan((Integer) data.get("tujuan_penggunaan"));
        tmp.setTipe_konsumen((Integer) data.get("tipe_konsumen"));
        tmp.setCluster((Integer) data.get("cluster"));
        tmp.setProductid((Integer) data.get("produk"));
        Float floatMinimalDP = Float.parseFloat(String.valueOf(data.get("minimaldp")));
        tmp.setMinimalDP(floatMinimalDP);
        ...
        RateMinimalDPRepository.save(tmp);
        HashMap<String, String> crunchifyMap = new HashMap<>();
        crunchifyMap.put("code", "1");
        crunchifyMap.put("message", "Input Minimal DP Berhasil !");
        return crunchifyMap;
    }
