...
public class JenisPembiayaanRestController {
    ...
    public Map<String, Object> getAllDataJenisPembiayaan(Principal principal, Pageable pageable,
        ...
        if (role.contains("USER")) {
            try {
                logger.info("Berhasil GET ALL DATA Jenis Pembiayan");
                crunchifyMap.put("jenisPembiayaan", jenisPembiayaanRepository.findAll());
                crunchifyMap.put("code", "1");
            } catch (Exception e) {
                logger.error("ERROR");
                response.setStatus(400);
                crunchifyMap.put("code", "0");
                crunchifyMap.put("message", "Gagal membuka Jenis Pembiayaan!");
            }
            return crunchifyMap;
        } else {
