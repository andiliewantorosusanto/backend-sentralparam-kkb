...
@CrossOrigin(origins = "*")
public class RatePerluasanAsuransiRestController {

    Logger logger = LoggerFactory.getLogger(RatePerluasanAsuransiRestController.class);
    ...
    public Map<String, Object> getAllDatarateperluasanasuransi(Principal principal, Pageable pageable,
        ...
        if (role.contains("USER")) {
            try {
                ...
                crunchifyMap.put("datarateperluasanasuransi", rateperluasanasuransiRepository.getListDataRatePerluasanAsuransi());
    ...
    public HashMap<String, String> viewrateperluasanasuransi(HttpServletResponse response) {
    ...
    public Map<String, Object> getAllDataApprovalrateperluasanasuransi(Principal principal, Pageable pageable,
            HttpServletResponse response) {
        Map crunchifyMap = new HashMap<String, Object>();
        ...
        if (role.contains("USER")) {
            try {
                logger.info("Berhasil GET ALL DATA RATE BUNGA");
                crunchifyMap.put("datarateperluasanasuransi", rateperluasanasuransiRepository.getListBucketApprovalRatePerluasanAsuransi(pageable));
                crunchifyMap.put("code", "1");
            } catch (Exception e) {
                logger.error("ERROR");
                response.setStatus(400);
    ...
    public HashMap<String, String> editrateperluasanasuransi(@RequestBody Map<String, Object> data, Principal principal,
            HttpServletResponse response) {
        HashMap<String, String> crunchifyMap = new HashMap<>();
        return crunchifyMap;
    }
    ...
    // NOTE Input Rate Asuransi
    @PostMapping(path = "/input", consumes = "application/json")
    public HashMap<String, String> insertRateAsuransi(@RequestBody Map<String, Object> data, Principal principal,
            HttpServletResponse response) throws JarException, JSONException, ParseException {
                System.out.println("masuk input rate perluasan asuransi");
                modelPerluasanAsuransi tmp = new modelPerluasanAsuransi();
                SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                //ArrayList<modelRateAsuransi> listModelRateAsuransi = new ArrayList<>();
                tmp.setNamaSkema((String) data.get("nama_skema"));
                tmp.setWilayah((Integer) data.get("wilayah"));
                tmp.setStartOTR((Integer) data.get("start_otr"));
                tmp.setEndOTR((Integer) data.get("end_otr"));
                tmp.setTipeAsuransi((Integer) data.get("tipe_asuransi"));
                ...
                tmp.setLoan_type((Integer) data.get("loan_type"));
                tmp.setJenis_kendaraan((Integer) data.get("jenis_kendaraan"));
                tmp.setJenis_pembiayaan((Integer) data.get("jenis_pembiayaan"));
                tmp.setStartyear((Integer) data.get("start_year"));
                tmp.setEndyear((Integer) data.get("end_year"));
        ...
        //listModelRateAsuransi.add(tmp);
        rateperluasanasuransiRepository.save(tmp);
        HashMap<String, String> crunchifyMap = new HashMap<>();
        crunchifyMap.put("code", "1");
        crunchifyMap.put("message", "Input Rate Perluasan Asuransi Berhasil !");
        return crunchifyMap;
    }
