...
public class JenisKendaraanRestController {
    ...
    public Map<String, Object> getAllDataJenisKendaraan(Principal principal, Pageable pageable,
        ...
        if (role.contains("USER")) {
            try {
                logger.info("Berhasil GET ALL DATA Jenis Kendaraan");
                crunchifyMap.put("jenisKendaraan", jenisKendaraanRepository.findAll());
                crunchifyMap.put("code", "1");
            } catch (Exception e) {
                logger.error("ERROR");
                response.setStatus(400);
                crunchifyMap.put("code", "0");
                crunchifyMap.put("message", "Gagal membuka Jenis Kendaraan!");
            }
            return crunchifyMap;
        } else {
